...
impl YOLO26Predictor {
    ...
    fn naive_bilinear_opencv(img: &RgbImage, dst_w: u32, dst_h: u32) -> RgbImage {
        ...
        for y in 0..dst_h {
            for x in 0..dst_w {
                ...
                let mut rgb = [0u8; 3];
                for c in 0..3 {
                    let val = p11[c] as f32 * (1.0 - dx) * (1.0 - dy)
                        + p21[c] as f32 * dx * (1.0 - dy)
                        + p12[c] as f32 * (1.0 - dx) * dy
                        + p22[c] as f32 * dx * dy;
                    rgb[c] = (val + 0.5) as u8;
                }
    ...
    fn preprocess(&self, img: &DynamicImage) -> Result<(Array4<f32>, PreprocessMeta)> {
        ...
        let resized = Self::naive_bilinear_opencv(&img.to_rgb8(), new_unpad_w, new_unpad_h);
    ...
    pub fn predict(
        ...
        let kept_indices = Self::nms(&candidates, iou);
    ...
    fn nms(candidates: &Vec<([f32; 4], f32, usize, Array1<f32>)>, iou_thresh: f32) -> Vec<usize> {
        ...
        while !indices.is_empty() {
            ...
            kept.push(current);
            indices.retain(|&idx| {
                Self::calculate_iou(&candidates[current].0, &candidates[idx].0) <= iou_thresh
            });
        }
        kept
    }

    fn calculate_iou(b1: &[f32; 4], b2: &[f32; 4]) -> f32 {
