...
mod tests {
    ...
    fn test_model_consistency() -> Result<()> {
        ...
        for (img_name, expected_dets) in &expected_map {
            ...
            let img = image::open(&img_path).map_err(|e| eyre!("Failed to load {img_name}: {e}"))?;
            ...
            if actual_dets.len() != expected_dets.len() {
                all_errors.push(format!(
                    ...
                    expected_dets.len(), actual_dets.len()
                ));
            }
            ...
            for (i, (actual, expected)) in actual_dets.iter().zip(expected_dets.iter()).enumerate() {
                ...
                if actual.tag != expected.tag {
                    all_errors.push(format!("[{det_id}] Tag mismatch: {} != {}", expected.tag, actual.tag));
                }

                // 2. Score
                if (actual.score - expected.score).abs() > FLOAT_EPSILON {
                    all_errors.push(format!("[{det_id}] Score mismatch: {} != {}", expected.score, actual.score));
                }

                // 3. Bounding Box
                let actual_bbox = [actual.bbox.x1, actual.bbox.y1, actual.bbox.x2, actual.bbox.y2];
                for (j, (a, e)) in actual_bbox.iter().zip(expected.bbox.iter()).enumerate() {
                    if (a - e).abs() > FLOAT_EPSILON {
                        all_errors.push(format!("[{det_id}] BBox coord[{j}] mismatch: {e:.2} != {a:.2}"));
                    }
                }

                // 4. Mask
                check_mask(&det_id, actual.mask.as_ref(), expected.mask_stats.as_ref(), &mut all_errors);
            }
        }
    ...
    fn check_mask(id: &str, actual: Option<&object_detector::ObjectMask>, expected: Option<&MaskStats>, errors: &mut Vec<String>) {
        match (actual, expected) {
            (Some(a), Some(e)) => {
                let active: usize = a.data.iter().map(|b| b.count_ones() as usize).sum();
                if a.width != e.width || a.height != e.height {
                    errors.push(format!("[{id}] Mask dimensions mismatch: {}x{} != {}x{}", e.width, e.height, a.width, a.height));
                }
                if active != e.active_pixels {
                    let diff = (active as f64 - e.active_pixels as f64).abs() / e.active_pixels as f64;
                    errors.push(format!("[{id}] Mask pixels mismatch: {} != {} ({:.2}% diff)", e.active_pixels, active, diff * 100.0));
                }
            }
